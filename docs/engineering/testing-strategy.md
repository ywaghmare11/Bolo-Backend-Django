...
> **Last updated:** 2026-07-30 — 2 more real bugs found + fixed (label-grounding scope, word-level keyword gap), catalog grown to 80 cases total. Previously: 2026-07-23 — Global Search automated test catalog: 65 cases across 3 suites, 2 real bugs found + fixed, 2 silent coverage gaps (assigneeLabel matching, due filter) found via manual field audit + fixed, plus confirmed scope decisions (Draft/Cancelled/Done_D included by design; Comments/Evidence/Voice transcript stay deferred). Previously: 2026-06-20 — audit log added to V1 scope (W63 resolved). Web-first; native mobile testing deferred with the Mobile PRD. No rejection / `requiresEvidence` tests (W-C1 resolved).
...
## Global Search — Automated Test Catalog (added 2026-07-23, grown 2026-07-30)

Full test suite for the Global Search feature (`bolo-backend/src/search/`, `SearchRepository.ts`, `services/search/globalSearch.service.ts`, `docs/api/global-search-ai-contract.md`), run directly against the service/controller/repository layers — same "bypass HTTP+JWT via a direct call" pattern already established by `scripts/test-label-scenarios.ts`. **80/80 passing** as of this run (grown from 74/74 on 2026-07-23 — 2 more real bugs found + fixed, see below).
...
9. **Label-grounding scope bug (F19c, found 2026-07-30 via direct user testing).** `getLabels()` in `globalSearch.service.ts` scoped the "Known labels" fuzzy-correction grounding to `createdBy: userId` — correct for the label *management* UI ("each user sees only labels they created"), wrong for search. A task's `mainLabel` is shared/visible to both assigner and assignee (search itself already matches it unconditionally), so a user who is only the *assignee* on a task — not the label's creator — never got that label into their own grounding list, and a typo like "naak" for "NAAC SSR" silently failed for them while working fine for the label's creator. Fixed by widening `getLabels()` to include any label used as a `mainLabel` on a task the user is assigner *or* assignee on (mirrors `SearchRepository`'s own task-visibility rule), unioned with labels the user created themselves.
10. **Word-level keyword gap (F19d, found 2026-07-30).** Once a fuzzy label correction fired, only the label's *full* name (e.g. "NAAC SSR") was ever added as a search keyword — an exact match for a task's `mainLabel` field, but useless against a sticky note or title/description that only mentions "NAAC" on its own, without "SSR". Real report: a user's own sticky note literally said "NAAC related" and a "naak" search still returned zero sticky results even after fix #9. Fixed by having `closestFuzzyLabel()` also return the *specific matched word* (not just the whole label), added as its own additional keyword alongside the full label name.

Full suite re-verified green after both — catalog now **80/80** (up from 74/74), the extra cases coming from F18/F19-series additions covering this pair plus adjacent label-fuzzy scenarios (not individually tabulated in the Suite 3 table above — see the script itself, `test-search-fuzzy.ts`, for the exhaustive current list).
