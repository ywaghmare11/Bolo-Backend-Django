...
6. **Second actor source — PlatformAdmin routes** (built Phase 15b, mirrors the original's `actorSource` extension in `docs/architecture/system-design.md`). A config row with `actor: "platform_admin"` is attributed from the `admin_token` cookie (`decode_admin_cookie` in `apps/common/request_identity.py`), not the tenant-user `token` cookie: `actor_type = PLATFORM_ADMIN`, `actor_id = None` (a `PlatformAdmin` has no `User` FK target), and the operator's identity goes in `AuditLog.metadata` as `{platformAdminId, platformAdminEmail}`. `AuditLog.tenant` (required) is resolved from the *target* tenant — `tenant_id_kwarg` names the URL path param for the member routes, `tenant_id_resolver` reads the response body for create-tenant. `write_audit_log_task` takes optional `actor_type`/`metadata` kwargs for this; omitting them keeps the original `USER`/`SYSTEM` inference for every other route. A `metadata_response_fields` config key (used by the bulk-import route) copies named fields from `response.data["data"]` into `AuditLog.metadata` — so the import run's `{created, updated, skipped}` counts land on the audit row, not only the HTTP response.
