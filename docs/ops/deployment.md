...
- **Retention: 14 days** across all signal types (logs, metrics, traces) — corrected 2026-07-23; the original "13-month metrics" claim was wrong, 13-month is Cloud Pro only.
- **Region: India — AWS ap-south-1 (Mumbai)**, confirmed 2026-07-23 against Grafana's own docs. DPDP compliant — no cross-border data transfer. Stack region must be explicitly set at creation; Grafana Cloud does not support changing it afterward. Note in `deployment-proposal-client.md` item 4 referenced a US/EU concern — now resolved; no client re-approval needed.
...
- **Image tagging (redesigned 2026-07-19, second pass):** single scheme — `:vX.X.X` only, no `:latest`/`:<commit-sha>` in parallel. Auto-computed as the next patch version on every deploy (`v1.2.3` → `v1.2.4`), pure semver, no environment name in the tag. `bolo-backend`/`bolo-web` version independently.
- **Retention:** ECR lifecycle policy keeps last 4 images per repo (unchanged from the original Step 2 setup — deliberately kept low for staging, decided 2026-07-19; a deeper retention window is a PROD-specific decision, not staging's). Since rollback now redeploys an existing image rather than rebuilding, this directly bounds how far back rollback can reach — staging's `action: rollback` can only go back 4 deploys, accepted as fine given staging's disposable, actively-changing nature.
- **Rollback (redesigned 2026-07-19):** explicit `workflow_dispatch` input, not implicit ref-selection. `action: rollback` (vs `deploy`) + optional `rollback_version` (defaults to the version immediately before current if left empty). **No rebuild** — pulls and redeploys the already-existing ECR image for that version, near-instant compared to a full build. `action: deploy` computes and uses the next version tag *before* building, but only pushes the actual git tag after the deploy is confirmed successful. See `docs/ops/staging-runbook.md` "Backend Handoff Checklist" item 3 for the full mechanism.
...
- **Compose file split, one per repo (redesigned 2026-07-19):** was originally one shared `docker-compose.staging.yml` (`bolo-backend` + `bolo-web` + `alloy`) living only in `bolo-backend`'s repo — `bolo-web`'s workflow could only ever download it from S3, never push its own changes, and depended on `bolo-backend` having deployed at least once first. Fixed at the root, not patched around: split into two independently-owned files. **`bolo-backend`'s repo** keeps `docker-compose.staging.yml` (now just `bolo-backend` + `alloy` — alloy stays bundled here, it isn't part of either app's release cycle), synced to S3 as `docker-compose.backend.yml`. **`bolo-web`'s repo** has its own `docker-compose.staging.yml` (just `bolo-web`), synced as `docker-compose.web.yml` — via a new "Sync deploy config to S3" step mirroring `bolo-backend`'s. Each workflow's SSM command now runs `docker compose -f <its-own-file> -p <bolo-backend|bolo-web> pull/up` — explicit `-f` (neither file is the Compose-default name anymore) and explicit `-p` (pins the project name so the two stacks can never share Compose's own network/label bookkeeping, even by accident). Net effect: either repo can deploy first, either can push its own config changes, and a bad edit or deploy on one side structurally cannot reach the other's *containers* (create/pull/up/down are fully isolated). **Networking is deliberately NOT isolated, corrected same-day (2026-07-19) after a live break:** the initial assumption that `bolo-web` had no internal Docker-network dependency on `bolo-backend` was wrong — `nginx.staging.conf` does `proxy_pass http://bolo-backend:3000/...` for `/api/` and `/ws`, a static (non-variable) upstream nginx resolves once at startup; when the split gave each project its own default network, that resolution broke, and nginx doesn't just disable the route, it fails to start at all — surfaced live as a Cloudflare `521` with the container crash-looping (`host not found in upstream "bolo-backend"`). Fix: both compose files now join one shared **external** network, `bolo-net` (`sudo docker network create bolo-net`, one-time, must exist before either file's `up` runs — `external: true` won't create it). Lifecycle stays isolated per project; only the network is deliberately shared. No cross-project `depends_on` exists to enforce startup order (Compose can't reference a service outside its own file) — `restart: unless-stopped` covers it in practice (nginx crash-loops until `bolo-backend`'s container exists on the network, then stabilizes), a self-heal rather than a real guarantee. Full narrative in `docs/ops/staging-runbook.md`.

- **nginx DNS re-resolution — required pattern (fixed 2026-07-23):** A bare `proxy_pass http://bolo-backend:3000/...` resolves that hostname once at nginx startup and caches the IP forever. `bolo-backend` gets a new IP on `bolo-net` on every redeploy (Docker assigns fresh IPs) — `bolo-web`'s nginx then silently proxies to the old, dead IP until it is also restarted, causing a live 502 on every `/api/` call for as long as `bolo-web`'s container stays up. **Fix in `nginx.staging.conf`:** `resolver 127.0.0.11 valid=10s` (Docker's embedded DNS) + a variable in `proxy_pass` (`set $backend_upstream bolo-backend:3000; proxy_pass http://$backend_upstream;`) — nginx re-resolves on the configured TTL instead of caching forever. **Paired requirement:** using a variable in `proxy_pass` disables nginx's automatic "replace matched location prefix" path rewriting, so every `proxy_pass` that also strips a path prefix needs an explicit `rewrite ... break` before it (otherwise nginx sends the literal path text from the config, truncating the actual request path). Both fixes are already in `nginx.staging.conf` (PRs #76/#78, 2026-07-23). **Any future nginx+Docker reverse-proxy config for this project must include both.**
...
**Gap found and fixed (2026-07-17):** the AWS/S3 vars (`AWS_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `S3_BUCKET`, `S3_PREFIX`, `S3_VOICE_BUCKET`, `S3_EVIDENCE_BUCKET`, `S3_PROFILE_BUCKET`, `S3_BROADCAST_BUCKET`) were present in `bolo-backend/.env` but never added to `docker-compose.yml`'s `backend.environment` block — Compose only injects vars a service's `environment:` block explicitly lists, having them in `.env` is not sufficient on its own. Symptom: any presigned-upload feature (broadcast image, evidence, profile picture) run via `docker compose up` returned a URL from the presign call (signing happens locally, no network round-trip, so it "succeeds" even with missing credentials) but every actual PUT/GET against that URL 403'd once S3 validated the signature. Fixed by adding the same `${VAR}` passthrough pattern already used for `SMTP_*` — see the entry above.
...
1. Identify the failing deployment (check error tracking / logs) and which service regressed (`bolo-backend`, `bolo-web`, or both)
2. GitHub → Actions → `Deploy to AWS Staging` (whichever repo regressed) → **Run workflow**, keeping "Use workflow from" on `staging` (not a tag)
3. Set **`action: rollback`**. Leave **`rollback_version`** empty to go to the version immediately before the current one, or specify one explicitly (e.g. `v1.3.0`) to jump further back
4. **Run workflow** — no rebuild, pulls and redeploys that version's already-existing ECR image (near-instant compared to a full build). Every deploy is auto-tagged (`vX.X.X`, pure semver), so there's always something to roll back to without needing to remember to tag manually — see `docs/ops/staging-runbook.md` "Backend Handoff Checklist" item 3
5. If a DB migration was applied: run rollback migration (tested on staging first)
6. Document incident in the ops runbook
