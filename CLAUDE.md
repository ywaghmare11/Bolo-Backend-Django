...
- [x] **Due-date reminder sweep** (`apps/tasks/tasks.py`, `apps/sticky_notes/tasks.py`) — `TASK_DUE_TODAY`/`TASK_DUE_TOMORROW`/`TASK_OVERDUE` (+ real `OVERDUE` status transition, both directions) and `StickyNote.REMINDER_FIRED`, both one-shot via persisted DB guards, `autoretry_for=(SMTPException,)`. ROADMAP.md Phase 8's remaining piece. **AI Nudges (recurring skip-cap/escalation feed, `GET /nudges`) still not built** — a separate, larger feature. See `changelog.md` 2026-08-12 (2)
