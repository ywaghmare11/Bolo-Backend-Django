...
- [ ] `django-prometheus` metrics, Sentry — scoped out this pass, user's explicit call: both need a real external account (metrics scraper / Sentry DSN) this dev sandbox doesn't have, so they'd be inert config with nothing to verify against, unlike `structlog` which is fully self-contained and demoable today.
...
### 15c — Multi-format bulk import (Excel `.xlsx` + CSV + JSON) — a small ETL pipeline — ✅ built 2026-08-29, see `changelog.md` 2026-08-29 (5)

**Built:** `POST /platform-admin/tenants/:tenantId/members/import` (multipart `file`). New `apps/platform_admin/etl.py` — `extract()` (one DataFrame regardless of format; xlsx via `pd.read_excel(engine="openpyxl")`, csv `utf-8-sig` → `latin-1` fallback, json array or `{members:[...]}`) → `transform()` (alias-normalised headers, `BOOL_MAP` coercion, vectorised `str.match`/`isin` validation, within-file dedup by email keeping the last, formula-injection neutralisation for `= + - @`, `MAX_ROWS`/required-column whole-file guards) → Load in `PlatformAdminTenantService.bulk_import_members` (idempotent `update_or_create` by email, 100/transaction). `pandas` + `openpyxl` added (numpy rides in as a pandas dep, not pinned separately), both lazy-imported. `MEMBERS_BULK_IMPORTED` audit row wired via a new `metadata_response_fields` config key that folds `{created, updated, skipped}` into `AuditLog.metadata`. 30 new tests (ETL unit + API E2E for all three formats). **Department assignment deliberately out of scope** for import. Reference design below kept for the write-up.
