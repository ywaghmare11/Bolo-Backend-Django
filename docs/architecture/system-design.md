...
> **Still open:** W64 (readiness indicators), W65 (routing approach — zero-router vs thin shell), W19 (org-role permission model). W63 resolved (2026-06-20): audit log IS in V1 — see schema V1.1. **W24 updated (2026-07-23):** search is no longer OpenSearch — see §10 and `docs/api/global-search-ai-contract.md`. Provider selections (OTP, scheduler) are locked to AWS — see §10. All other open questions resolved — see `docs/product/open-questions-web-v1.md`.
...
**Broadcast notification retention sweep (built 2026-08-07):** a third scheduled job (`broadcastNotificationRetentionSweep.job.ts`, every 30 min, same cadence as the due-date sweep) hard-deletes `BROADCAST_POSTED` notifications whose broadcast is no longer active. Root cause it fixes: `BroadcastNotice` has a hard 24h `expiresAt`, but `Notification` rows for it never expired on their own — a user who hadn't opened the app in days would still have bell/panel entries for broadcasts that no longer exist anywhere reachable in the UI (`GET /broadcast-notices?view=received` already excludes both cases via `findActiveForAudience`'s `status: 'PUBLISHED', expiresAt: { gt: now }`), so clicking one always dead-ended on "No active notices right now" with no explanation. Live-DB testing while building the fix turned up a **second flavor of the same bug**: notifications for broadcasts deleted outright via `DELETE /broadcast-notices/:id` (an even harder dead end — the entity doesn't exist at all, not just expired) — of the 22 stale `BROADCAST_POSTED` rows found in the local dev DB, 18 pointed at expired-but-still-existing broadcasts and a further 25 pointed at fully deleted ones. Implemented as an allow-list, not a delete-list: `BroadcastNoticeRepository.findActiveIds()` returns currently-`PUBLISHED`-and-unexpired broadcast ids; `NotificationRepository.deleteStaleBroadcastNotifications()` deletes every `BROADCAST_POSTED` row whose `entityId` isn't in that set — a deleted broadcast's id simply never appears in the active list at all, so both cases collapse into the same `notIn` query with no separate "was it deleted" check needed (no real FK exists between `Notification.entityId` and `BroadcastNotice.id` — `Notification` is polymorphic across task/broadcast/sticky_note — so this is necessarily a two-step lookup-then-delete, not a single joined query). Deletes rather than marking expired — there's no "view an expired/deleted broadcast" screen anywhere to route to either way, so keeping the row serves no purpose once its target is gone. **Verified against a live local DB:** ran the sweep manually against real accumulated test data — deleted exactly 43 of 47 `BROADCAST_POSTED` rows (18 expired + 25 deleted-broadcast), left the 4 notifications for a freshly-published still-active broadcast untouched; a second run afterward deleted 0 (idempotent). Known residual gap: up to a 30-minute window between a broadcast going stale and the next sweep run where its notification can still be clicked into the same dead end — accepted, matches the existing tolerance the due-date sweep already has at the same cadence.

**Delete path gets immediate cleanup too (same day):** the sweep's 30-min window turned out to be too slow for the one case that already has a clear synchronous hook — a sender deleting their own broadcast is a deliberate, single-request action, not a passive time-based one, so it doesn't need to wait for the next sweep run. Reported directly: a sender deleted a notice they'd just posted, saw it correctly vanish from their own dashboard widget, but it kept showing in the notification panel (including after a refresh) since nothing had told the notification layer the broadcast was gone. `deleteBroadcastNoticeService` now calls `NotificationRepository.deleteByEntity('broadcast', broadcastId)` (new, generic-by-entity delete — not the sweep's allow-list `notIn` query, since this needs exactly one specific broadcast's rows gone, immediately) right after `repo.delete()`. Verified live: published a broadcast to the entire tenant (12 recipients notified), deleted it, confirmed all 12 notification rows were gone in the same request cycle — no wait for the sweep. The sweep job itself is unchanged and still runs independently, both as the only mechanism for the expiry case (nothing to synchronously hook — a broadcast doesn't have a discrete "just expired" event, it's simply true after enough time passes) and as a safety net for any notification whose broadcast was removed some other way.
...
**AI Nudge scheduler architecture — current state as of 2026-08-18 (supersedes the original 2026-07-06/07-10 redesign below this point):** one shared scheduler rule (15-min tick, `aiNudgeSweep.job.ts`) invokes `runAiNudgeSweep()`, which first computes a shared `fastTrackRecipients` set once per tick, then evaluates both AI Nudge trigger conditions:

1. **Follow-up** (Task only — **scope narrowed 2026-07-13**, **2 conditions**, own 6h gap, runs continuously — no office-hours gate): (b) accepted-no-progress, (c) unanswered comment (assignee owes the reply). Conditions (a) not-accepted, (d) `DONE_A`-awaiting-`DONE_D`, (e) subtasks-done-awaiting-parent-close were **removed entirely 2026-07-13**, not just their buttons — those are irreversible actions better taken deliberately from the task itself, and they're already covered by the general Notification panel. **The assigner is out of scope for Follow-up entirely** now — (d)/(e) were the only assigner-facing conditions, and both are gone. Skip counter tracked for visibility only, no cap, no escalation.
2. **Due-proximity** (Task only — **StickyNote and Broadcast dropped 2026-07-13**; StickyNote's own one-shot `REMINDER_FIRED` reminder still exists independently, see the due-date sweep above), own 3h gap, runs continuously: dedup-check against the re-fire interval (per-recipient) → notify. **Skip is a user-clicked button** (`POST /nudges/:id/skip`), never auto-incremented by the sweep — the sweep only *reads* the counter to decide escalation. Cap: 3 due-today / 1 overdue. **No blocking behavior (removed 2026-07-13):** Skip is never disabled/hidden at cap and the panel is never force-closed — at cap the card just shows a plain warning; the user can keep skipping. Escalation is still real, independent of the UI: each tick, if `skipCount >= cap`, not yet escalated, and the task hasn't reached at least `DONE_A` → one-time in-app+email to the assigner, guarded by `NudgeSkipCounter.escalatedAt`.

**First-login-of-the-day fast-track (added 2026-07-16, activity-source corrected 2026-08-18):** a recipient's first eligible nudge of the day (Follow-up or Due-Proximity, whichever they hit first) bypasses that type's interval gate and fires on the very next 15-min sweep tick, instead of potentially waiting up to 3h/6h purely by interval-timing coincidence. `runAiNudgeSweep()` computes `fastTrackRecipients` once per tick (`NudgeRepository.findFirstNudgeFastTrackRecipients`) and passes the same set into both `runFollowup()`/`runDueProximity()`, so a fast-tracked user's first sweep surfaces everything currently eligible at once. A user drops out of the set as soon as their first nudge fires (their own new notification's `createdAt` is now "today"); normal interval gating resumes for the rest of the day.
  - **Originally (2026-07-16) based on `User.lastLoginAt >= startOfToday`.** Found broken 2026-08-18: the JWT cookie lives 7 days and BOLO ships as a PWA, so a user who logs in once and just keeps using the installed app never triggers another login — `lastLoginAt` goes stale after day one, silently killing the fast-track for every day after the first.
  - **Fixed 2026-08-18 — reads `User.lastActiveAt` instead.** New field, touched (throttled to once per 10 min per user via a conditional `updateMany`, not an in-memory cache — stays correct across multiple backend instances) on **any authenticated request** in `requireAuth`. Reflects actual daily usage regardless of login recency. See `lastActiveAt` in `domain-model.md`'s User table.

**Periodic is retired — removed from the `NotificationType` enum entirely** (not just deprecated), as of migration `20260709182552_nudge_skip_counter_and_periodic_retirement`. Do not reintroduce.

**Office-hours gating removed entirely (2026-07-06).** Was 9am–6pm IST; didn't generalize across verticals/timezones/usage patterns. The sweep runs 24/7 — cadence is purely each type's own elapsed-time gap (Follow-up 6h → ~4 fires/day; Due-proximity 3h → ~8 fires/day, before the fast-track's once-daily bypass).

All intervals/caps/the fast-track's 10-min throttle are **configurable constants in code, not admin UI or a DB table, for the initial build.**

**Cross-type dedup, keyed on recipient+entity not just type (W84, corrected 2026-07-10):** before creating a `Notification` row, check whether *any* `AI_NUDGE_*` type already fired for this same `recipientId`+`entityType`+`entityId` within the cooldown window (`NotificationRepository.findMostRecentAiNudgeForRecipientAndEntity`). Now that Task is the only entity type, this check applies uniformly with no batching exception needed (originally written to also guard against Broadcast's many-recipients-per-entity shape, which no longer exists in-scope).

**`entityType` is always `'task'` as of the 2026-07-13 scope-down.** A Subtask is a `Task` row with `parentTaskId` set, not a separate table, but — unlike before 07-13 — nudges no longer distinguish it (`entityType: 'subtask'` was retired along with the Subtask-specific handling); a subtask is just another task from the assignee's point of view.

**Skip counters — schema built 2026-07-10 (W94 resolved), narrowed 2026-07-13.** Originally keyed on `(userId, entityType, entityId, nudgeKind)` to handle Broadcast's many-recipients-per-entity shape. With Broadcast out of scope, migration `20260712200810_nudge_scope_task_only` dropped `userId` from the key (now `(tenantId, entityType, entityId, nudgeKind)`) and added `lastShownAt` for feed rotation (see the 5-item feed cap below). See `domain-model.md` row 8c/8d for the full shape and `docs/api/api-spec.md` §11 for the `GET /nudges` / `POST /nudges/:id/skip` / `POST /nudges/skip-all` endpoints built on top of it.

**Nudge panel UI — scope narrowed 2026-07-13:** single unified list, feed capped at **5 items**, priority-then-rotation ordering (uses `NudgeSkipCounter.lastShownAt` to rotate which eligible items surface when there are more than 5). **No blocking behavior** — Skip and the panel's close button are never disabled, regardless of cap/last-chance state (this was a real bug fixed 2026-07-13: Skip was previously hidden at last-chance, and the panel's close button was separately non-functional — both are now unconditional). The `GET /nudges` feed re-validates every row against current entity state on every call and auto-resolves (marks read) anything whose underlying condition no longer holds — never trusts what was true when the notification originally fired. Dedup on `entityId:type` before building candidates, so the same task never occupies more than one of the 5 slots.

**Broadcast received-list staleness (added 2026-07-26):** editing a live `PUBLISHED` broadcast (message/image) dispatches no `Notification` — it's a content edit, not a new event — so an already-open receiver has no push-style signal that anything changed. Rather than invent one (which would conflate "data freshness" with "user alerting," and add a new `NotificationType` just to force a cache refresh), `useBroadcastNotices('received')` (`bolo-web/src/hooks/useBroadcastNotices.ts`) polls every 15s via TanStack Query's `refetchInterval` — the same convention already used for `useNotificationCount`/`useAssignedTaskAlert` — so an open receiver view picks up edits (and the "Edited" badge, see `prd.md` §7) without a manual reload. `view=sent` is unaffected; it's the sender's own screen and is already fresh from their own action.
...
**Comments (2026-07-25):** first case in this build requiring a real schema change, not just config — `AuditAction` had no `COMMENT_*` values at all (unlike `DOCUMENT`/`TENANT`, which arrived pre-added by other PRs). Added `COMMENT_CREATED`/`COMMENT_UPDATED`/`COMMENT_DELETED` via migration, plus a new `AuditLog.entityType` value `COMMENT` and `AuditableModel: 'comment'`. 3 config rows: `POST /tasks/:id/comments` → `COMMENT_CREATED`, `PATCH`/`DELETE /tasks/:id/comments/:cid` → `COMMENT_UPDATED`/`COMMENT_DELETED`. Same path-param gotcha as evidence (documented in the tech-playbook pattern) applied again to the create route — `:id` in `/tasks/:id/comments` is the task's id, not the new comment's — caught by unit-verifying the matcher before shipping, same discipline as evidence. `COMMENT` rows are TOP-only for now, same `findEntityAssignerId()` gap as `DOCUMENT` — now two entity types sharing this exact limitation, worth resolving together if non-TOP assigner access to Task-scoped children is ever needed. Live-verified full success path against a real DB (create → update → delete), unlike evidence/profile-picture which only got failure-path verification due to missing S3 credentials.
...
> **bolo-backend-django (Phase 15b, 2026-08-29):** the same extension is now built here. `apps/common/audit_middleware.py` grew the second actor path; the three config keys map to `actor: "platform_admin"` (↔ `actorSource`), `tenant_id_kwarg` / `tenant_id_resolver` (↔ `tenantIdParam` — URL kwarg for the member routes, response field for create-tenant), and the already-existing `id_resolver_post` with `_response_data_field("tenantId"|"userId")` (↔ `entityIdResponseField`). `write_audit_log_task` gained `actor_type`/`metadata` kwargs; the operator's identity rides in `AuditLog.metadata` as `{platformAdminId, platformAdminEmail}` since `actor_id` must stay null. `TENANT_CREATED`/`MEMBER_ADDED`/`MEMBER_REMOVED` wired; `MEMBERS_BULK_IMPORTED` waits on Phase 15c. No `PlatformAdmin` service/view calls the audit layer — same generic-observer discipline.

**StickyNote retention sweep (added 2026-07-23):** a separate scheduler rule, daily tick, `stickyNoteRetentionSweep.job.ts` — hard-deletes any `StickyNote` where `dueAt IS NOT NULL AND dueAt + interval '3 days' <= NOW()`. Deliberately its own job rather than folded into `runAiNudgeSweep()`: it's a data-lifecycle/deletion concern, not a notification, and a daily cadence is enough (unlike the 3h/6h nudge gaps, there's no user-facing urgency to deleting same-day). No soft-delete — matches the rest of the system's no-undo posture (W67). Reuses the existing EventBridge + Lambda scheduler infra, no new provider decision.
...
**Post-login URL (added 2026-08-09):** `verify-otp`'s response includes `tenantSlug` (`Tenant.urlSlug`, domain-model.md) alongside `name`. `bolo-web` uses these to navigate to `/{tenantSlug}/{firstName}` immediately after login (`buildAppPath()`, `deriveFirstName()` in `src/utils/appUrlPath.ts`) — the address bar reflects the logged-in tenant/user from that point on. **Cosmetic only**, never an authorization boundary: every API call is still scoped by `tenantId` from the JWT cookie, never from the URL. `PrivateRoute` (`src/routes/privateRoutes.tsx`) validates the URL's `tenantSlug`/`userSlug` params against the actual session on every private-route render and silently redirects to the caller's own real path on any mismatch (stale bookmark, hand-edited URL, or a different user logging in on a shared device) — it never trusts the URL for anything beyond display. This supersedes the 2026-07-18 `/integrate18/varun/` fixed-prefix scheme (docs/ops/staging-runbook.md) — `bolo-web` now serves from domain root; nginx needs only a generic SPA fallback, not a hardcoded prefix.
...
| Search | PostgreSQL + standalone AI query-understanding layer (OpenAI `gpt-4o-mini`) | **Changed 2026-07-23 (W24 updated) — no longer OpenSearch.** Built for Task/Subtask + Sticky Note only, not "every entity." OpenSearch was priced out (₹2,000–9,000/month unbudgeted) — see `tech-playbook/decisions/database.md`. Full design: `docs/api/global-search-ai-contract.md` |
