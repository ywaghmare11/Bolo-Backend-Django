...
**AI Nudge scheduler architecture — redesigned 2026-07-06, built 2026-07-10, scope narrowed 2026-07-13, login-aware fast-track 2026-07-16 (current state of record — supersedes all earlier detail below):** one shared scheduler rule (15-min tick, `aiNudgeSweep.job.ts`) invokes `runAiNudgeSweep()`, which evaluates both AI Nudge trigger conditions each run:
1. **Follow-up** (Task only — Subtask no longer distinguished, Broadcast/StickyNote never applied here — **2 conditions**, own 6h gap, runs continuously, no office-hours gate): accepted-no-progress; unanswered-comment (**assignee-only** — only fires when the assignee owes the reply; if the assignee posted last and is waiting on the assigner, no nudge fires, since the assigner is out of scope for Follow-up entirely). The 3 conditions that required Accept Task/Mark Complete actions (not-yet-accepted, `DONE_A`-awaiting-`DONE_D`, subtasks-done) were **removed entirely, not just their buttons**, 2026-07-13 — those are irreversible actions the user should take deliberately from the task, already covered by the general Notification panel. Each condition dedup-checked independently per-recipient; tracks a skip counter in `NudgeSkipCounter` for visibility/rotation only — no cap, no escalation.
2. **Due-proximity** (Task only, same narrowing), own 3h gap, runs continuously: dedup-check against the re-fire interval (per-recipient) → notify. **Skip is a user-clicked button** (`POST /nudges/:id/skip`), not something the sweep auto-increments — the sweep only *reads* the counter to decide escalation. Cap: 3 due-today / 1 overdue. **No blocking behavior (removed 2026-07-13):** Skip is never disabled/hidden at cap and the panel is never forced closed — at cap the UI just shows a warning; the actual one-time escalation to the assigner still fires for real, independent of the UI (`NudgeSkipCounter.escalatedAt` guards against repeats). **Add Comment resolves the nudge for that cycle** (fixed 2026-07-13 — the classifier now checks for a comment posted after the notification fired, since Due-Proximity's own eligibility check never looked at comments the way Follow-up's does).

**Feed is capped at 5 total** (`GET /nudges`, added 2026-07-13) — Due-Proximity fills first (`Task.priority` order, P1 highest), Follow-up fills remaining slots (same priority order, then oldest-`lastShownAt`-first as the rotation tiebreaker). See `domain-model.md` row 8c/8d for the full mechanics.

**First-login-of-the-day fast-track (added 2026-07-16):** the interval gate above has no notion of whether the user was ever online — a user with short, irregular sessions could go a full day without a session ever overlapping the exact moment an interval elapsed. `NudgeRepository.findFirstNudgeFastTrackRecipients(startOfToday)` returns users who logged in today (`User.lastLoginAt`) but haven't received any AI Nudge yet today; computed once per sweep tick and passed into both Follow-up/Due-Proximity, bypassing the interval gate once for anyone in the set. Self-limiting — the moment it fires, the user's own new notification is "today," so the next computation naturally excludes them. No schema addition; built from existing `lastLoginAt`/`Notification.createdAt`. Day boundary is plain server-local midnight, matching `findTasksNeedingDueProximityNudge`/`getThisWeekRange`'s existing convention.

**Periodic is retired — removed from the `NotificationType` enum entirely** (not just deprecated), as of migration `20260709182552_nudge_skip_counter_and_periodic_retirement`. It was a batched, per-user "you have N open tasks" summary with no per-task action. Once Follow-up gained per-task action buttons and lost its own skip-cap, there was no remaining structural difference between the two types. Do not reintroduce.

**Office-hours gating removed entirely.** Was 9am–6pm IST; assumed a single institution's business hours, which doesn't generalize across BOLO's multiple verticals, timezones, or individual login/usage patterns. The sweep runs 24/7 — each type's cadence is purely its own elapsed-time gap (Follow-up 6h → ~4 fires/day; Due-proximity 3h → ~8 fires/day), on top of which the first-login fast-track above can bypass the gate once per user per day. **Known consequence, accepted:** Task due-proximity's caps (3/1), sized assuming ~3 fires/day inside a 9h window, now exhaust same-day within a few hours at 8 fires/day across 24h — an overdue task (cap 1) starts at last-chance immediately, zero grace skips. Matches "overdue is urgent," just faster than the original model assumed. Exact per-user configurable scheduling is a future feature-flag-service item — times stay as hardcoded constants for now.

All intervals/caps are **configurable constants in code, not admin UI, for the initial build.**

**Cross-type dedup, keyed on recipient+entity not just type (W84, corrected 2026-07-10):** before creating a `Notification` row, check whether *any* `AI_NUDGE_*` type already fired for this same `recipientId`+`entityType`+`entityId` within the cooldown window (`NotificationRepository.findMostRecentAiNudgeForRecipientAndEntity`). Recipient-scoped rather than entity-only — originally mattered because Broadcast has many recipients per entity; kept as the general pattern even though Broadcast itself is out of scope now, since it's strictly more correct than entity-only for any future many-recipient entity type.

**`entityType` is always `"task"` now (Subtask distinction removed 2026-07-13).** Was previously stamped `'subtask'` vs `'task'` based on `parentTaskId` — dropped as part of the scope narrowing, since a Subtask is treated identically to a Task for nudge purposes now (the assignee doesn't experience a meaningful difference).

**Skip counters — schema built 2026-07-10 (W94 resolved), simplified 2026-07-13.** `Task.dueProximitySkipCount`/`dueProximityEscalatedAt` are gone. Replaced by a generic `NudgeSkipCounter` table. The 2026-07-10 build keyed it on `(userId, entityType, entityId, nudgeKind)` specifically for Broadcast's many-recipients-per-entity problem; once Broadcast (and Subtask/StickyNote) dropped out of scope on 2026-07-13, `userId` was removed again — back to simple `(entityType, entityId, nudgeKind)` keying, since every remaining candidate (Task) has exactly one assignee. The same migration added `lastShownAt`, used by the Follow-up rotation above. See `domain-model.md` row 8c/8d for the full shape and `docs/api/api-spec.md` §11 for the `GET /nudges` / `POST /nudges/:id/skip` / `POST /nudges/skip-all` endpoints built on top of it.
