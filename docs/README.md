...
| `ops/deployment-django.md` | **This project's own** deploy + CI/CD runbook (added 2026-08-27, Phase 13). Written from scratch for the Django/gunicorn/Celery stack — the Docker image, `docker-compose`, the `ci.yml` pipeline, and the (reference/disabled) AWS ECS Fargate `deploy.yml`. Supersedes the inherited `deployment.md`/`staging-runbook.md` for anything Django-specific. |
