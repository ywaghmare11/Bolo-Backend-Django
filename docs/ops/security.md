...
- [x] **Tenant offboarding (bolo-backend-django, Phase 15e, 2026-08-29):** `Tenant.status` (`ACTIVE` / `SUSPENDED`) via `PATCH /platform-admin/tenants/:id` (`SUPER_ADMIN`). While `SUSPENDED`, `AuthService` raises `403 TENANT_SUSPENDED` from `request-otp` / `verify-otp` / `refresh` — combined with the 15-minute access token this locks the tenant out within one token lifetime with no per-request DB check. Due-date + AI-nudge Celery sweeps filter `tenant__status=ACTIVE`; `POST .../members[/import]` return `409 TENANT_SUSPENDED`. All data retained; fully reversible. `TENANT_SUSPENDED` / `TENANT_REACTIVATED` audit rows via the same `actor: "platform_admin"` path. No hard-delete — a purge is a separate export-first step (W58).
