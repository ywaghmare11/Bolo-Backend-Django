...
**Created:** 2026-06-06 | **Last updated:** 2026-07-25 — §23 added: W110–W111, broadcast audience targeting gaps surfaced while regression-testing the composer's Department/Role pickers. Previous update 2026-07-21: §22 added, W102–W109, accepted voice-command scope limitations surfaced while building the voice command manual test plan (`docs/engineering/testing-strategy.md`). Previous update 2026-06-17 (round 4): resolved W66/W67/W68 (voice CRUD scope = all 5 entities, mandatory confirm-before-delete, flow diagrams built into `diagrams.html` §9–10); clarified W20 is dev discretion, not a contradiction; clarified W16 is "client needs to review Rhushabh's answer first," not a rejection of it.
**Status:** Almost all items resolved. Genuinely open: **W15** (task card fields — pull from Figma), **W43** (voice privacy — low priority), **W60** (billing UI/flow), **W64** (readiness indicators), **W65** (routing approach), **W110** (no true "send to everyone" broadcast option), **W111** (should broadcast targeting be department-scoped to the sender). See **🚩 Major Flags** and **Critical Gaps** near the end. **W63 resolved (2026-06-20):** Audit log is in V1 scope — full AuditLog table in schema V1.1. **W100 resolved (2026-07-16):** transactional email is AWS SES, client-confirmed.
...
| **W35** | Member invite flow post-onboarding? | ✅ Resolved, **see 🚩 flag above** — **Email link + OTP login.** New users are added on our side **via an admin portal** ("we would update new user through our admin portal for the backend") — not purely the Excel/CLI re-import we'd previously assumed (D19). Login flow: user enters email → we check it exists in the tenant's DB → if yes, send OTP. (2026-06-17) ✅ **Built (2026-07-15) — see W98.** The "admin portal" is the new backend-only `PlatformAdmin` (superadmin) API — Email+OTP login, creates tenants, adds/removes members in any tenant. No bolo-web UI yet (backend-only for MVP, per explicit scope decision). |
...
| **W99** | Can a main task reach DoneD if one of its subtasks is CANCELLED (not DONE_D)? | ✅ **Resolved 2026-07-22.** Yes — a `CANCELLED` subtask counts as resolved for the parent's completion gate, alongside `DONE_D`. Rationale: `CANCELLED` is terminal (W12/W13) and can never transition to `DONE_D`, so requiring literal `DONE_D` on every subtask would have permanently blocked completion of any parent with a cancelled subtask. Implemented in `TaskRepository.allSubtasksDoneD` (`bolo-backend/src/repositories/TaskRepository.ts`) — gate now checks `status NOT IN (DONE_D, CANCELLED)`. |
| **W100** | If a task is `OVERDUE` and its due date is edited to today-or-later, should status revert automatically? | ✅ **Resolved 2026-07-22 — bug fix, not a new ambiguity.** Yes — `updateTaskService` (`bolo-backend/src/services/task/updateTask.service.ts`) now reverts `OVERDUE → IN_PROGRESS` (if `acceptanceStatus = ACCEPTED`) or `OVERDUE → OPEN` (if not yet accepted) when `dueDate` is patched to today-or-later. Previously nothing ever moved a task out of `OVERDUE` — only the `runOverdue` sweep (`dueDateSweep.service.ts`) ever set it, one-way, so a task with a future due date could stay stuck showing `OVERDUE` indefinitely. Found via user report; verified against a real task in both acceptance states. |
...
| **W114** | Should there be a shared, unified intent-classification + jargon-glossary module (covering both CS/MCA and Education-vertical acronyms) reused by both the voice-command flow (`voice/intent.js`) and Global Search (`search/searchClassify.ts`), instead of each maintaining its own separate/incomplete grounding? | 🟡 **Open — explicitly deferred (2026-07-30).** Surfaced while root-causing the NAAC/naak search bugs: `voice/intent.js`'s jargon glossary is CS/MCA-only (no Education-vertical terms — Dean/HoD/Faculty acronyms like NAAC have no equivalent list), and `searchClassify.ts` is deliberately standalone (per explicit instruction not to touch `intent.js`), so the two modules can't share even the parts that would generalize. User: "we need a separate thing for a intent and jargons, we can look into it later" — tracked here, not scheduled. |
...
**Superseded 2026-07-13 — client scoped the feature down substantially** after reviewing the full build: Task-only (no Subtask/StickyNote/Broadcast), Follow-up cut to 2 assignee-only conditions (no Accept/Mark Complete anywhere), no blocking behavior (Skip never disabled, panel never forced closed), feed capped at 5 with priority-then-rotation ordering. See `domain-model.md` row 8c/8d and `changelog.md` 2026-07-13 for the full spec. Built on a new `feature/ai-nudge-scoped` branch (both repos) — `feature/ai-nudge` kept intact rather than modified in place, so the fuller original build is still there to pick from if any of this gets walked back. **Both backend and frontend merged 2026-07-13** (`feature/ai-nudge-scoped`, both repos) — frontend has since switched off mock data onto the real scoped endpoints. Two real bugs found via live testing after merge and fixed on `feature/ai-nudge-scoped` before it closed: `Skip` button was hidden at last-chance instead of always visible (contradicted the "no blocking" scope-down itself), and the panel's `[×]` close button was non-functional.

**W112 — First-login-of-the-day fast-track (added 2026-07-16).** User complaint: with office-hours gating removed and the pure 3h/6h interval gate, a user who logs in for the first time that day could wait hours for their first nudge purely by interval-timing coincidence — worse if their usage is bursty (a few hours on, a few hours off). ✅ **Resolved — Option 1 of 4 presented, approved by user.** A user's first eligible nudge each day bypasses the interval gate and fires on the next 15-min sweep tick. Built on `feature/ai-nudge-login-aware` (backend only), PR #52, merged. Initially keyed off `User.lastLoginAt`.

**W113 — `lastLoginAt`-based fast-track breaks for PWA sessions (found + fixed 2026-08-18).** User caught the flaw directly: BOLO ships as a PWA with a 7-day session cookie, so most users never trigger a second real "login" after their first day — `lastLoginAt` goes stale, silently disabling the fast-track from day two onward. ✅ **Resolved — new `User.lastActiveAt` field**, touched (throttled to once per 10 min per user) on any authenticated request via `requireAuth`, tracks real daily usage instead of login events. Fast-track query switched to read it. Considered a heartbeat-endpoint alternative (FE pings while tab visible) — rejected as unnecessary precision for what the fast-track actually needs (coarse "active today, yes/no"), and it would've required frontend changes across both repos for no behavioral gain here. Built on `feature/ai-nudge-activity-tracking`, PR #115. Live-tested end-to-end: real HTTP request through `requireAuth`, plus the actual `runFollowup()`/`runDueProximity()` sweep functions with seeded nudges inside their gates, confirming the fast-track path re-fires while the non-fast-tracked path stays correctly gated.
...
## Section 19 — Platform Admin / Superadmin (2026-07-15)

Prompted by a direct ask: need a way to register new tenants and manage users across tenants, without the client's own tenant-scoped `TOP` role (which can't act outside its own tenant). Directly resolves the old **W35** flag ("admin portal for the backend") into a concrete design — see `docs/architecture/domain-model.md` ("PlatformAdmin" section) and `docs/api/api-spec.md` §20 for the full shape.

| # | Question | Status |
|---|---|---|
| **W98** | How should a cross-tenant "superadmin" actor be modeled, and should it replace the existing public `POST /onboard/register` endpoint? | ✅ **Resolved & built (2026-07-15).** New standalone `PlatformAdmin` model — not a `User`, not a `TenantMembership` role, no `tenantId` anywhere, sits outside RLS/tenant scoping entirely (`User.tenantId` is required/non-nullable per W56, so a cross-tenant actor can't be a `User`). Auth: Email+OTP, fully parallel to tenant-user login (separate `PlatformAdminOtpCode` table, separate `admin_token` cookie, separate JWT payload with no `tenantId`/`roleLevel`) — no self-registration, provisioned only via `scripts/seedPlatformAdmin.ts` (ops-only). The old public, unauthenticated `POST /onboard/register` — discovered mid-implementation to be undocumented, pre-existing code, explicitly commented "the only public write endpoint in the system" — is **removed**; tenant creation now requires `requirePlatformAdmin` via `POST /platform-admin/tenants`. Also added: `GET /platform-admin/tenants` (list), `POST`/`DELETE /platform-admin/tenants/:tenantId/members[/:userId]` (add/remove a user in any tenant). Every platform-admin action writes an `AuditLog` row (`actorType: PLATFORM_ADMIN`). Backend-only for MVP — no bolo-web UI. |

---

## Section 23 — Broadcast Audience Targeting Gaps (2026-07-25)

Surfaced while regression-testing the Broadcast Notice composer's audience picker (department/role selection) against a real multi-department tenant.

| # | Question | Status |
|---|---|---|
| **W110** | **There is no way to publish a broadcast with zero audience targeting** — `audienceDeptId` + `audienceRoleLevel` cannot both be unset (`prd.md` §7: *"there is no 'send to everyone' without explicitly selecting the scope"*). In practice this means a sender who wants to reach literally 100% of the tenant must either (a) pick one specific Role Level, which reaches that role only, or (b) manually select every individual department, which excludes any member not assigned to a department (e.g. a Director/Dean/Admin sitting above all departments — `departmentId` is optional per `domain-model.md:97`, so this is a real, expected case, not bad data). Neither path is actually "everyone." | ⏳ **Open — asking client/product owner**, but a **stop-gap is already live (2026-07-25)** ahead of their answer: `bolo-web`'s `hasAudience` check (`CreateBroadcastNotice/hooks.ts`) and `bolo-backend`'s `publishBroadcastNotice.service.ts` (branch `fix/broadcast-entire-institution-standalone`) both now treat explicitly-selected "Entire Institution" (empty deptIds + null roleLevel) as a complete, valid, publishable audience — reaching every tenant member including anyone with no department. `prd.md` §7 and `api-spec.md` §10 updated to match. **Still need:** should V1 keep this, or was "you must always pick a scope" actually intentional — revert both changes if so. |
| **W111** | **Broadcast targeting has no department-scoping tied to the sender.** `canBroadcast` (`TenantMembership`) is a single tenant-wide binary flag (`security.md:31-33`) — a member of one department with `canBroadcast: true` can target *any* department or role in the tenant, including ones they have no connection to (e.g. a Computer Science HoD broadcasting to Civil). `createBroadcastNoticeService.ts` only validates that target department IDs belong to the same tenant, never that they relate to the sender's own department. This matches the PRD's explicit "no hierarchy enforcement anywhere else" principle (`prd.md:77-79`) but was never explicitly confirmed for Broadcast specifically. | ⏳ **Open — asking client/product owner.** Should department-level senders (HoD) be restricted to broadcasting only within their own department, while only tenant-wide roles (Dean/Director) can target any department — or is the current unrestricted behavior intended? |
...
*Last updated: 2026-07-25 — §23 (broadcast audience targeting gaps, W110–W111) added. Previous: 2026-07-21 — §22 (voice command known limitations, W102–W109) added. Previous: 2026-06-17 (Rhushabh's answers from the client Google Sheet, round 2) | Next: incorporate the user's verbal PRD update notes, then update `docs/product/prd.md` and propagate confirmed changes into `domain-model.md` / `system-design.md` / `api-spec.md` / `schema.prisma` / `design-session.md`.*
