...
### 5.7 AI Behaviour
- **AI Nudge** (implemented as scheduled cron triggers, not ML) has **two trigger types. Scope narrowed 2026-07-13 (client-directed) to Task only** — Subtask, StickyNote, and Broadcast all dropped; a Subtask is no longer distinguished from Task at all:
  1. **Follow-up** — fires every 6h, continuously (no office-hours gate — see below). **2 conditions, both assignee-only**, each with its own action button, none capped/escalating:
     - Task accepted, no progress update since → assignee → `Add Comment`
     - Comment posted and the **assignee** owes the reply (assigner posted last) → assignee → `Add Comment`. If the assignee posted last and is waiting on the assigner, no nudge fires — the assigner is out of scope for Follow-up entirely.
     - The 3 conditions requiring `Accept Task`/`Mark Complete` actions (not-yet-accepted, `DONE_A` awaiting `DONE_D`, all-subtasks-done) were **removed entirely, not just their buttons** — those are irreversible actions the user should take deliberately from the task itself, already covered by the general Notification panel. Both remaining conditions track a lifetime skip counter in DB for visibility/rotation, but enforce **no cap and no escalation**.
  2. **Due-date proximity** — fires every 3h, continuously, Task only. **Add Comment + Skip + Open**. **Skip is a user-clicked button** (`POST /nudges/:id/skip`), not something the sweep increments automatically. Cap: 3 for due-today, 1 for overdue — in practice an overdue task starts at last-chance immediately (zero grace skips). **No blocking (removed 2026-07-13):** Skip is never disabled/hidden at cap, the panel is never forced closed — at cap the card just shows a warning that skipping will escalate to the assigner; Skip stays clickable regardless. **Add Comment resolves the nudge for that cycle** (fixed 2026-07-13 — checks for a comment posted after the notification fired). **Escalation is still real**, independent of the UI: next sweep, if the task still hasn't reached at least `DONE_A` → one-time in-app + email to the **assigner**, never repeats. If it reached `DONE_A`, it naturally drops out of the sweep — no escalation. The assignee is only ever held to `DONE_A`; `DONE_D` is the assigner's responsibility.
- **Feed capped at 5 total** (added 2026-07-13) — Due-Proximity fills first (`Task.priority` order, P1 highest), Follow-up fills remaining slots (priority, then oldest-shown-first rotation so candidates that don't fit still surface over time).
- **First-login-of-the-day fast-track (added 2026-07-16):** a user who's logged in today but hasn't received any nudge yet today bypasses the interval gate once, on the next sweep tick — solves the case where short/irregular sessions could otherwise miss the exact moment an interval elapsed. Self-limiting, no extra config — built from existing `lastLoginAt`.
- **Office-hours gating removed entirely (2026-07-06).** Was 9am–6pm IST; dropped since it assumed one institution's business hours, which doesn't generalize across verticals, timezones, or individual login patterns. The sweep now runs continuously — cadence comes purely from each type's own gap (Follow-up 6h/~4 fires per day, Due-proximity 3h/~8 fires per day), on top of which the first-login fast-track can bypass the gate once per user per day. Times themselves stay fixed in code for now; a per-user configurable schedule is a future feature-flag-service item, not built yet.
...
- **Nudge configuration rules:**
  ...
  - **Cross-type duplicate suppression (W84, corrected 2026-07-10):** dedup keyed on `recipientId`+`entityType`+`entityId`, not just `entityType`+`entityId` — recipient-scoped rather than entity-only (originally mattered for Broadcast's many-recipients-per-entity shape; kept as the general pattern even though Broadcast itself is out of scope now).
  - **Daily nudge cap — still descoped.** Not being built.
  - **Panel behavior (redesigned 2026-07-06, scope narrowed 2026-07-13):** single unified nudge list — max 5 items, priority-ordered, Due-Proximity-first-then-Follow-up-fill (see above), no more Type/Entity filters (moot once Entity narrowed to Task only). **No blocking** — Skip and panel close are never disabled, matching the Due-Proximity behavior above.
...
| 8b | **AI Nudge — Follow-up** | System | Assignee only (accepted-no-progress, unanswered-comment) — assigner is out of scope for Follow-up entirely as of 2026-07-13 | **Scope narrowed 2026-07-13 — 2 conditions**, Task only (Subtask no longer distinguished, Broadcast/StickyNote never applied). Action: `Add Comment`. The 3 conditions needing `Accept Task`/`Mark Complete` were removed entirely, not just their buttons. Fires every 6h, continuously (no office hours; first-login-of-the-day fast-track can bypass the gate once, added 2026-07-16). Skip counter tracked in DB for both, but **no cap, no escalation** on either. |
| 8c | **AI Nudge — Due Date Proximity** (`AI_NUDGE_DUE_PROXIMITY`) | Assignee routinely, + assigner once cap exhausted (escalation, one-time) | **Task only** (scope narrowed 2026-07-13 — Subtask/StickyNote/Broadcast dropped). Fires every 3h, continuously (no office hours; fast-track as above). Cap: 3 due-today / 1 overdue, escalates to assigner if not at least `DONE_A` by next window. **No blocking (2026-07-13)** — Skip is never disabled at cap, panel never forced closed; at cap the card shows a warning only. Add Comment resolves the nudge for that cycle. Feed capped at 5 total, Due-Proximity-first-then-Follow-up-fill (priority-ordered). |
