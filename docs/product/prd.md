...
- A main task can only be marked complete when **all its subtasks (if any) are resolved** — each one is either `DONE_D` or `CANCELLED` (W99, resolved 2026-07-22). A cancelled subtask does **not** block the parent, since `CANCELLED` is terminal and can never become `DONE_D`. If a subtask isn't completed before the parent's due date, the parent cannot be completed and enters Overdue.
- **Editing the due date on an `OVERDUE` task reverts its status** (W100, resolved 2026-07-22) — if the new due date is today-or-later, status goes back to `IN_PROGRESS` (if already accepted) or `OPEN` (if not yet accepted). Before this fix, nothing ever moved a task out of `OVERDUE` once the scheduled sweep set it, even after the due date was pushed forward.
...
- **Auto-delete on overdue expiry (added 2026-07-23):** a Sticky Note with `dueAt` set is **hard-deleted 3 days after `dueAt` passes** (i.e. `dueAt + 3 days <= NOW()`), regardless of pin state. Sticky Notes with no `dueAt` are never auto-deleted. No warning/grace-extension step — deletion is silent, consistent with "no undo anywhere in the system" (W67).

### 5.7 AI Behaviour
- **AI Nudge** (implemented as scheduled cron triggers, not ML) has **two trigger types, Task only** (**scope narrowed 2026-07-13, client-directed** — Subtask/StickyNote/Broadcast all dropped; the current state supersedes the original 2026-07-06/07-10 redesign):
  1. **Follow-up** — fires every 6h, continuously (no office-hours gate). **2 conditions**, both assignee-only, neither capped/escalating:
     - (b) Task accepted, no progress update since → assignee → `Add Comment`
     - (c) Comment posted, assignee owes the reply (assigner posted last) → assignee → `Add Comment`
     - Conditions (a) not-yet-accepted, (d) `DONE_A`-awaiting-`DONE_D`, (e) subtasks-done-awaiting-parent-close were **removed entirely 2026-07-13**, not just their buttons — those are irreversible actions better taken deliberately from the task itself, and already covered by the general Notification panel. **The assigner is out of scope for Follow-up entirely** — (d)/(e) were the only assigner-facing conditions.
  2. **Due-date proximity** — fires every 3h, continuously. Task only (`IN_PROGRESS`/`OVERDUE` or due-today, already-accepted): `Add Comment` + `Skip` + `Open`. **Skip is a user-clicked button** (`POST /nudges/:id/skip`), never auto-incremented by the sweep. Cap: 3 for due-today, 1 for overdue. **No blocking behavior (removed 2026-07-13):** `Skip` is never disabled or hidden, even at cap — at cap the card shows a plain warning instead ("skip this and it'll be escalated to your assigner"), but the user can keep skipping past it. **Escalation is still real, independent of the UI**: next sweep, if `skipCount >= cap`, not yet escalated, and the task hasn't reached at least `DONE_A` → one-time in-app + email to the **assigner**, never repeats. Reaching `DONE_A` drops the task out of the sweep — no escalation. The assignee is only ever held to `DONE_A`, never `DONE_D`.
- **First-login-of-the-day fast-track (added 2026-07-16, corrected 2026-08-18):** a user's first eligible nudge of the day bypasses that type's interval gate and fires on the next 15-min sweep tick, rather than potentially waiting hours purely by interval-timing coincidence. Originally driven by `User.lastLoginAt`; **corrected 2026-08-18** to instead read a new `User.lastActiveAt` field (touched on any authenticated request, throttled to once per 10 min) — `lastLoginAt` only updates on real OTP login, and since BOLO ships as a PWA with a 7-day session cookie, a user who logs in once and keeps using the installed app was silently falling out of the fast-track after their first day.
- **Office-hours gating removed entirely (2026-07-06).** Was 9am–6pm IST; dropped since it assumed one institution's business hours, which doesn't generalize across verticals, timezones, or individual login patterns. The sweep now runs continuously — cadence comes purely from each type's own gap (Follow-up 6h/~4 fires per day, Due-proximity 3h/~8 fires per day, before the fast-track's once-daily bypass). Times themselves stay fixed in code for now; a per-user configurable schedule is a future feature-flag-service item, not built yet.
...
- **Nudge configuration rules:**
  - Nudge rules are configurable and extensible by an admin/configurator (future feature-flag service — not built yet, intervals/caps/fast-track throttle are hardcoded constants for V1).
  - Multiple nudge types can be active for the same task simultaneously.
  - **Cross-type duplicate suppression (W84, corrected 2026-07-10):** dedup keyed on `recipientId`+`entityType`+`entityId`, not just `entityType`+`entityId`. Now that Task is the only entity type, this applies uniformly with no batching exception needed (originally written to also guard Broadcast's many-recipients-per-entity shape, which is no longer in scope).
  - **Daily nudge cap — still descoped.** Not being built (distinct from the once-daily fast-track bypass above, which relaxes timing, not volume).
  - **Panel behavior — scope narrowed 2026-07-13:** single unified nudge list, feed capped at **5 items**, priority-then-rotation ordering. **No blocking behavior** — neither `Skip` nor the panel's close button is ever disabled, regardless of cap/last-chance state (this was a real bug fixed 2026-07-13: `Skip` was previously hidden at last-chance, and the close button was separately non-functional — both are now unconditional).
...
- **Audience Scope (2026-07-25 stop-gap, pending client/product owner confirmation — W110, `open-questions-web-v1.md` §23):** a broadcast **can** now be published with no Department or Role level set — this is treated as "Entire Institution," reaching every tenant member including anyone with no department assigned. Previously mandatory (no "send to everyone" without explicitly selecting a scope); relaxed because there was no way to reach 100% of a tenant otherwise (picking one Role missed other roles; picking every Department excluded members with no department). Revert to mandatory-scope if the client says so.
...
- **Search** — Task/Subtask + Sticky Note only, fuzzy/typo-tolerant via an AI query-understanding layer (updated 2026-07-23, narrowed from the original "everything the user can access" — see W24 in `open-questions-web-v1.md` and `docs/api/global-search-ai-contract.md`). A person's name is a match field (surfaces their tasks), not a separate result type.
...
| 8b | **AI Nudge — Follow-up** | System | Assignee only | **Scope narrowed 2026-07-13 (supersedes the original 2026-07-06 redesign): Task only, 2 conditions** — accepted-no-progress, unanswered-comment (assignee owes reply), both `Add Comment`. The 3 assigner-facing/not-accepted conditions were removed entirely, not just their buttons. Fires every 6h, continuously (no office hours). Skip counter tracked in DB for visibility only — **no cap, no escalation**. **First-login-of-the-day fast-track (2026-07-16, corrected 2026-08-18 to use `User.lastActiveAt` not `lastLoginAt`)** — see §5.7. |
| 8c | **AI Nudge — Due Date Proximity** (`AI_NUDGE_DUE_PROXIMITY`) | System | Assignee routinely, **+ assigner once cap exhausted** (escalation, one-time) | **Scope narrowed 2026-07-13: Task only** (StickyNote/Broadcast dropped from this type — StickyNote keeps its own separate one-shot `REMINDER_FIRED`, see row 13). Fires every 3h, continuously (no office hours). Cap: 3 due-today / 1 overdue, escalates to assigner if not at least `DONE_A` by next window. **No blocking (removed 2026-07-13):** `Skip` and `Add Comment`/`Open` all stay available even past cap — a plain warning replaces the old disabled-button behavior. Same fast-track as row 8b applies. |
...
| Search service | PostgreSQL `ILIKE` + AI query-understanding layer (not OpenSearch — dropped 2026-07-23 for cost, see `tech-playbook/decisions/database.md`) | Task/Subtask + Sticky Note search (§9.3) |
