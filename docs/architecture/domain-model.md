...
> **Last synced:** 2026-08-29 (bolo-backend-django) — Phase 15 slices: 15a `PlatformAdmin.role` RBAC (`SUPER_ADMIN`-only enum in the `admin_token` JWT, `HasPlatformAdminRole([...])` factory), 15b `AuditLog` wiring for `PlatformAdmin` actions (middleware second actor path), 15c `.xlsx`/`.csv`/`.json` member bulk-import, **15e `Tenant.status` offboarding** (`ACTIVE`/`SUSPENDED` + `suspendedAt`/`suspensionReason`; `PATCH /platform-admin/tenants/:id`; suspends cut login/refresh + Celery sweeps + member-add). New `AuditAction.TENANT_SUSPENDED`/`TENANT_REACTIVATED`. See the `Tenant` and `PlatformAdmin` field tables below. Still deferred: 15d SPA, hard tenant purge. Previously 2026-08-23: `PlatformAdmin`/`PlatformAdminOtpCode` core built (OTP auth, create/list tenant, add/remove member; bulk-import + audit-log wiring deferred — see CLAUDE.md). `Tenant.url_slug` also built (nullable, since the field predates its assignment endpoint by a day). `User.lastActiveAt` remains not built — only relevant to the AI Nudge fast-track this project already declined to build. Previously 2026-08-22: re-synced from upstream through its 2026-08-18 state. Re-applied the Django-only findings below that upstream's own docs still don't carry (verified against upstream *source*, not doc prose, same discipline as the 2026-08-03 sync): `JargonWord` entity, `Task.evidenceRequired`, member reactivation, Evidence delete-access narrowing. Previously (2026-07-15 upstream): added `PlatformAdmin` + `PlatformAdminOtpCode` (superadmin, W35 resolved). Previously (2026-06-27): added `VoiceRecording` entity (W37 cascaded): stores raw SDK transcript, audio S3 key, language, duration, and AI confidence score per task.
...
| role | enum | ✅ | **bolo-backend-django addition (ROADMAP.md Phase 15a) — upstream's `PlatformAdmin` has no role field.** RBAC tier *within the operator team*, a separate axis from `OrgRoleLevel`. Only `SUPER_ADMIN` exists today (default); `SUPPORT_ADMIN`/`VIEWER` deferred until the operator team actually needs to split access. Carried in the `admin_token` JWT (`{ adminId, email, isPlatformAdmin, role }`) and enforced by a `HasPlatformAdminRole([...])` permission-class factory that mirrors `HasOrgRole`. `migrate` backfills existing rows to `SUPER_ADMIN`; the auth layer falls back to this column for tokens minted before the claim existed. |
| createdAt | timestamp | ✅ | |
| updatedAt | timestamp | ✅ | |
...
| urlSlug | string | ✅ upstream / — here | Unique. Client-chosen at tenant creation (`POST /platform-admin/tenants`, built here 2026-08-23), URL-safe (`^[a-z0-9]+(-[a-z0-9]+)*$`, 2-40 chars, `400 INVALID_URL_SLUG` if malformed) — drives the post-login URL path `/{urlSlug}/{firstName}` (system-design.md §4.3). **Cosmetic only** — never used for tenant scoping/authorization, which stays keyed on `id` from the JWT everywhere else. Added 2026-08-09 upstream; field built here 2026-08-22 (migration `0003`), assignment endpoint built 2026-08-23. **Stays nullable in this project** (unlike upstream's required field) — any tenant created before this endpoint existed, or via `seed_dev_data`/factories, has no slug; `POST /auth/verify-otp`'s `tenantSlug` returns `null` for those. |
| vertical | enum | ✅ | `EDUCATION` \| `CA_CS` |
| status | enum | ✅ | **bolo-backend-django addition (ROADMAP.md Phase 15e) — no upstream equivalent.** `ACTIVE` \| `SUSPENDED`, default `ACTIVE`. Operator offboarding: `SUSPENDED` cuts all login/refresh for this tenant's users (`403 TENANT_SUSPENDED` on `request-otp`/`verify-otp`/`refresh`), stops its due-date + AI-nudge Celery sweeps, and rejects `POST .../members[/import]` (`409`). Every row is retained; the transition is fully reversible. Set only via `PATCH /platform-admin/tenants/:id` (`SUPER_ADMIN`). No hard-delete state — a purge is a separate, export-first step (W58). |
| suspendedAt | timestamp | — | Set when `status → SUSPENDED`, cleared on reactivate. |
| suspensionReason | string | — | Operator-entered ("non-payment", "customer offboarded"). Cleared on reactivate. Captured in the `TENANT_SUSPENDED` audit row's `before`/`after`. |
| createdAt | timestamp | ✅ | |
| updatedAt | timestamp | ✅ | |
...
- Platform Admin (cross-tenant, `actorType = PLATFORM_ADMIN`): `TENANT_CREATED`, `MEMBER_ADDED`, `MEMBER_REMOVED`, `MEMBERS_BULK_IMPORTED`, `TENANT_SUSPENDED`, `TENANT_REACTIVATED`
...
- Platform Admin (added 2026-07-17, cross-tenant/superadmin — `system-design.md` §2.6): `TENANT_CREATED`, `MEMBER_ADDED`, `MEMBER_REMOVED`, `MEMBERS_BULK_IMPORTED`; **bolo-backend-django Phase 15e adds** `TENANT_SUSPENDED`, `TENANT_REACTIVATED` (tenant offboarding, `entityType: 'TENANT'`, wired via the same `actor: "platform_admin"` middleware path)
